    ...
    async function getAssessmentForSkill(apiKey_UNUSED, transcript, skillName, rubricData, index, wl, DEBUG_PROMPT_FLAG) {
      ...
      if (!rubricData || !Array.isArray(rubricData.levels) || rubricData.levels.length === 0) {
        return attachDebug({
          ...
          _raw_model_output: [],
          _served_by: { judge: env.JUDGE_MODEL || "fireworks/mixtral-8x7b-instruct", coach: env.WRITER_MODEL || "openai/o4-mini" }
        });
      }
...
      const judgeResp = await withRetry(() => chatOpenRouter(env, {
  model: env.JUDGE_MODEL || "fireworks/mixtral-8x7b-instruct",
        temperature: 0.1,
        max_tokens: 8000,
        tools: [{
          type: "function",
...
improvements = backfillImprovementQuotes(improvements, levelChecks);
  ...
  judge_model: env.JUDGE_MODEL || "fireworks/mixtral-8x7b-instruct",
